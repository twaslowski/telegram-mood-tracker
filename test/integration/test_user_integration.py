...
def test_querying_nonexistent_user_returns_none(mongodb_user_repository):
    assert mongodb_user_repository.find_user(1) is None


@pytest.mark.asyncio
async def test_return_value_of_user_creation(update, mongodb_user_repository):
    user = await create_user(update, None)
    assert user.user_id == 1
    assert user.metrics is not None
    assert user.notifications is not None

    assert user == mongodb_user_repository.find_user(1)


@pytest.mark.asyncio
async def test_registration(update, mongodb_user_repository):
    # user does not exist
    assert mongodb_user_repository.find_user(1) is None
    ...
    # now it exists
    assert mongodb_user_repository.find_user(1) is not None
    assert mongodb_user_repository.find_user(1).metrics is not None
    assert mongodb_user_repository.find_user(1).notifications is not None
...
@pytest.mark.asyncio
async def test_update_user(update, mongodb_user_repository):
    # Given a user
    ...
    user.notifications = []
    mongodb_user_repository.update_user(user)

    # The user has no notifications when next retrieved from the database
    assert mongodb_user_repository.find_user(123).notifications == []


@pytest.mark.asyncio
@pytest.mark.skip(reason="As of now, auto-baseline is not enabled upon user creation")
async def test_update_user(update, mongodb_user_repository, configuration, application):
    # Given a user
...
@pytest.mark.asyncio
async def test_toggle_auto_baseline_happy_path(
    update, mongodb_user_repository, application
):
    # Given a user with all baselines defined and auto-baseline disabled (and time configured)
    await create_user(update, None)
    assert mongodb_user_repository.find_user(1).has_auto_baseline_enabled() is False
    ...
    assert mongodb_user_repository.find_user(1).has_auto_baseline_enabled() is True
...
async def test_toggle_auto_baseline_without_auto_baseline_time_configured(
    update, mongodb_user_repository, notifier
):
